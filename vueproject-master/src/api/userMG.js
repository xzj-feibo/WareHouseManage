...
// 角色管理-获取角色列表（无参）
export const roleListNoParms = (params) => { return req("post", "/api/Role/list/no/parms") };
...
// 公司管理-获取公司列表（无参）
export const deptListNoParms = (params) => { return req("post", "/api/Dept/list/no/params") };
// 公司管理-保存（添加编辑）
export const deptSave = (params) => { return req("post", "/api/Dept/save", params) };
// 公司管理-删除公司
export const deptDelete = (params) => { return axios.delete("/api/Dept/delete?ids=" + params + "&token=" + localStorage.getItem('logintoken')).then(res => res.data) };
